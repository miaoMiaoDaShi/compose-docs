...
- [Text Context Menu 自定义](./text-context-menu.md) - Compose 1.9 新增 `appendTextContextMenuComponents()` / `filterTextContextMenuComponents()`，向文本选择上下文菜单追加或移除菜单项，实现增强剪贴板、翻译、搜索等自定义功能。
### 列表
...
- [Scroll Indicator 原生滚动指示器](./scroll-indicator.md) - Compose 1.11 Foundation 新增原生 `Modifier.scrollIndicator` / `ScrollIndicatorState` API，替代 Accompanist 等第三方滚动条方案，支持标准化的滚动指示器自定义（LazyColumn / verticalScroll 等通用）。
...
- [Lookahead 动画可视化调试](./lookahead-animation-debugging.md) - Compose 1.11 新增 `LookaheadAnimationVisualDebugging` 和 `CustomizedLookaheadAnimationVisualDebugging` 调试 composable，实时显示预测动画的位置、进度和速度。
...
## 最近更新
- 2026-04-07：例行维护扫描 — 确认 **Compose 最新稳定 BOM 仍为 2026.03.01**（Compose 1.10.5），**Beta 为 1.11.0-beta02**（2026-03-25）。Compose 1.11 正式版尚待发布。新增 **[Text Context Menu 自定义](./text-context-menu.md)**：收录 Compose 1.9（August 2025 Release）新增的 `Modifier.appendTextContextMenuComponents()` 和 `Modifier.filterTextContextMenuComponents()` API，详解追加/过滤上下文菜单项的基本用法、组合使用模式、TextContextMenuItem 数据类说明，以及增强剪贴板/翻译/搜索等实用场景。其余扫描到的知识点（Compose August 2025 2D Scrolling、Rich Text Transformations、LazyLayout 性能改进；December 2025 Pausable Composition 默认启用、性能与 Views 持平；Compose Multiplatform iOS Stable 1.8.0 等）均已在现有文档中覆盖，无需额外修改。
...
- 2026-04-06：例行维护扫描 — 确认 **Compose 最新稳定 BOM 仍为 2026.03.01**（Compose 1.10.5），**Beta 为 1.11.0-beta02**（2026-03-25）。Compose 1.11 正式版尚待发布。**新增 [Scroll Indicator 原生滚动指示器](./scroll-indicator.md)**：详解 Compose 1.11 Foundation 新增的 `Modifier.scrollIndicator` / `ScrollIndicatorState` API，包括接口属性说明、基本用法、自定义样式（strokeWidth/cornerRadius/color/hoverColor/trackColor）、与 Accompanist 第三方滚动条的对比，以及 `ScrollIndicatorState.toScrollbarGeometry()` 几何计算示例，替代原有第三方方案。其余 Compose 1.11 四大更新（Modifier.visible()、LookaheadAnimationVisualDebugging、Shared Context for ComposeView、Scroll indicator）中除 Scroll Indicator 外均已在前序更新中完成文档化。
本次大幅更新 [Compose Multiplatform 跨平台开发](./compose-multiplatform.md)，新增四大 Compose Multiplatform 1.10.0 新特性：**WindowInsetsRulers**（窗口插入区数值读取 API，跨平台统一获取 Status Bar/Navigation Bar/Keyboard 高度）、**PredictiveBackHandler**（跨平台预测性返回手势，iOS/Desktop 终于支持，与 Android 系统级 Predictive Back 互补）、**原生互操作视图自动调整大小**（iOS/Desktop 原生视图嵌入 Compose 时框架自动管理布局，无需手动同步尺寸）、**PredictiveBackHandler + 依赖别名弃用 + AGP 9.0.0 支持**；同步更新文档版本标注。**Pausable Composition 分帧预取已在 Kotlin 2.3.20 中默认启用**（LazyLayout 滚动性能持续受益）。**新增 NavigationEvent/NavigationBackHandler 迁移章节**：随着 PredictiveBackHandler 在 CMP 1.10.0 中被弃用（底层事件系统已迁移至 NavigationEvent），文档新增了从 PredictiveBackHandler 到 NavigationBackHandler + rememberNavigationEventState 的完整迁移指南，包括 API 对照、进度监听方式变更、swipeEdge 获取方法和 iOS Info.plist 配置要求。其余知识点（Lookahead 动画可视化调试、CacheWindow 预取、Modifier.skipToLookaheadPosition 等）均已在现有文档中覆盖。
