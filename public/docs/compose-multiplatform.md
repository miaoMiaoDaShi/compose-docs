...
**已知问题修复（1.11.0-beta02，2026-04-10）：**
- ✅ 修复 `BasicTextField(TextFieldValue)` 和 `BasicTextField(TextFieldState)` 在 `usingNativeTextInput=true` 时偶发崩溃问题（自动补全/键盘建议应用后崩溃）
- ✅ 修复空 TextField 三次点击（triple tap）时崩溃问题
- ✅ 修复文本末尾换行时光标和选区句柄丢失问题
- ✅ 修复键盘快捷键 `Ctrl+Home`/`Ctrl+End`（`Cmd+Home`/`Cmd+End`）不生效问题
- ✅ 修复 iOS 15 上 `UIWindowSceneGeometry` 符号缺失导致的崩溃
- ✅ 修复 Autofill 上下文菜单项在 `BasicTextField` 中的显示异常

**版本要求：**
- iOS 15.0+（通过 Kotlin/Native 工具链自动处理）
- Compose Multiplatform 1.11.0-beta01+（基础功能）
- Compose Multiplatform 1.11.0-beta02+（上述问题修复）
...
**Kotlin 2.3.20 Web 要求（1.11.0-beta02 起）：**
从 Compose Multiplatform 1.11.0-beta02 起，**使用 Kotlin/JS 或 Kotlin/Wasm 时必须使用 Kotlin 2.3.20+**。此项要求确保 Web 目标与 Kotlin 编译器最新特性保持同步。

**版本要求：**
- Compose Multiplatform 1.11.0-beta01+（滚动改进）
- Compose Multiplatform 1.11.0-beta02+（Kotlin 2.3.20 要求）
...
Compose Multiplatform 1.10.0 将 **Hot Reload** 升级为 **v1.0.0 正式版**（已内置于 `org.jetbrains.compose` 插件，默认启用），这是桌面端开发效率的重大提升。
...
**版本历史：**
- **1.1.0-beta02**（随 Compose Multiplatform 1.11.0-beta02 内置，2026-04-10）：修复了 `Ctrl+Home`/`Ctrl+End` 快捷键问题、Desktop 对话框内存泄漏和场景关闭时任务清理问题
...
## Compose UI Tests v2 API（Compose Multiplatform 1.11.0-beta02）🧪

> 适用于：Compose Multiplatform 1.11.0-beta02+ / 非 Android 平台测试

Compose Multiplatform 1.11.0-beta02 引入了 **v2 Compose UI Tests API**，为非 Android 平台的测试体验带来显著改进。

**核心变更：**

| 变更类型 | 说明 |
|---------|------|
| **API 命名调整** | `runComposeUiTest` → `runComposeUiTestV2`（旧 API 已弃用） |
| **默认调度器** | v2 API 默认使用 `StandardTestDispatcher`（而非 `UnconfinedTestDispatcher`），测试行为更可预测 |
| **effectContext 可定制** | 支持自定义 `effectContext`，方便测试场景配置 |
...
- 新项目直接使用 v2 API
- 已有项目可在下次测试代码重构时迁移
- Android 平台仍使用标准 Jetpack Compose Testing API，不受影响
